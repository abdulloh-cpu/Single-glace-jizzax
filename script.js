const DB_KEYS = {
  MENU: "si_menu",
  ORDERS: "si_orders",
  ADMIN: "si_admin_creds",
};

const DEFAULT_MENU = [
  {
    id: "m1",
    name: "Qaymoqli klassik",
    price: 15000,
    image:
      "https://images.unsplash.com/photo-1497034825429-c343d7c6a68f?auto=format&fit=crop&w=600&q=60",
    active: true,
    category: "muzqaymoq",
    type: "simple",
  },
  {
    id: "m2",
    name: "Shokoladli premium",
    price: 18000,
    image:
      "https://images.unsplash.com/photo-1560008581-09826d1de69e?auto=format&fit=crop&w=600&q=60",
    active: true,
    category: "muzqaymoq",
    type: "simple",
  },
  {
    ...
    price: 20000,
    image:
      "https://images.unsplash.com/photo-1576506295286-5cda18df43e7?auto=format&fit=crop&w=600&q=60",
    active: true,
    category: "muzqaymoq",
    type: "simple",
  },
  ...
  {
    id: "m5",
    name: "Shokolad Double",
    price: 25000,
    image:
      "https://images.unsplash.com/photo-1563805042-7684c019e1cb?auto=format&fit=crop&w=600&q=60",
    active: true,
    category: "muzqaymoq",
    type: "simple",
  },
  ...
  },
  {
    ...
    name: "Kola",
    image:
      "https://images.unsplash.com/photo-1554866585-cd94860890b7?auto=format&fit=crop&w=600&q=60",
    active: true,
    category: "ichimliklar",
    type: "sized",
    sizes: [
      { label: "1L", price: 12000 },
      { label: "1,5L", price: 15000 },
      { label: "2L", price: 18000 },
    ],
  },
];
...
const SEED_V2_ITEMS = [
  ...
  {
    id: "f1",
    name: "Hot-dog bulichka",
    price: 17000,
    image:
      "https://images.unsplash.com/photo-1612392062631-94dd858cba88?auto=format&fit=crop&w=600&q=60",
    category: "yeguliklar",
    type: "simple",
  },
  {
    id: "f2",
    name: "Hot-dog bulichka (katta)",
    price: 20000,
    image:
      "https://images.unsplash.com/photo-1612392062631-94dd858cba88?auto=format&fit=crop&w=600&q=60",
    category: "yeguliklar",
  ...
  // ---- YEGULIKLAR: Lavash ----
  {
    ...
    price: 30000,
    image:
      "https://images.unsplash.com/photo-1626700051175-6818013e1d4f?auto=format&fit=crop&w=600&q=60",
    category: "yeguliklar",
    type: "simple",
  },
  {
  ...
  {
    id: "dr5",
    name: "Lipton (muzli choy)",
    price: 0,
    image:
      "https://images.unsplash.com/photo-1499638673689-79a0b5115d87?auto=format&fit=crop&w=600&q=60",
    category: "ichimliklar",
    type: "simple",
  },
  {
  ...
  // ---- ICHIMLIKLAR: sharbatlar ----
  {
    ...
    price: 0,
    image:
      "https://images.unsplash.com/photo-1600271886742-f049cd451bba?auto=format&fit=crop&w=600&q=60",
    category: "ichimliklar",
    type: "simple",
  },
  {
    id: "dr9",
    name: "Dena sharbat (Multifrukt)",
    price: 0,
    image:
      "https://images.unsplash.com/photo-1600271886742-f049cd451bba?auto=format&fit=crop&w=600&q=60",
    category: "ichimliklar",
  ...
  {
    id: "dr15",
    name: "Mineral suv (Hydrolife)",
    price: 0,
    image:
      "https://images.unsplash.com/photo-1553530666-ba11a7da3888?auto=format&fit=crop&w=600&q=60",
    category: "ichimliklar",
    type: "simple",
  },
  {
    ...
    price: 0,
    image:
      "https://images.unsplash.com/photo-1622543925917-763c34d1a86e?auto=format&fit=crop&w=600&q=60",
    category: "ichimliklar",
    type: "simple",
  },
  {
...
const DB = {
  ...
  updateMenuItem(id, changes) {
    ...
    if (idx !== -1) {
      menu[idx] = { ...menu[idx], ...changes };
      DB.saveMenu(menu);
    }
  },
  deleteMenuItem(id) {
    DB.saveMenu(DB.getMenu().filter((m) => m.id !== id));
  },
  getOrders() {
  ...
  getAdminCreds() {
    const raw = localStorage.getItem(DB_KEYS.ADMIN);
    if (!raw) {
  ...
  saveAdminCreds(creds) {
    localStorage.setItem(DB_KEYS.ADMIN, JSON.stringify(creds));
  },
};
...
function statusLabel(key) {
  const found = ORDER_STATUSES.find((s) => s.key === key);
...
function renderMenu() {
  const menu = DB.getMenu().filter(
    (m) => m.active !== false && (m.category || "muzqaymoq") === activeCategory,
  );
  const grid = document.getElementById("menuGrid");
  grid.innerHTML = "";
  if (menu.length === 0) {
    grid.innerHTML =
      "<div class=\"empty-state\">Bu bo'limda hozircha taom yo'q</div>";
    return;
  }
  menu.forEach((item) => {
    ...
    if (item.type === "sized") {
      ...
      card
        .querySelector(".add-sized-btn")
        .addEventListener("click", () =>
          addToCart(item.id, selectedSizeMap[item.id]),
        );
    } else {
      card.innerHTML = `
        <img src="${item.image}" alt="${item.name}" onerror="this.src='https://images.unsplash.com/photo-1497034825429-c343d7c6a68f?auto=format&fit=crop&w=600&q=60'">
...
function renderCart() {
  ...
  if (keys.length === 0) {
    list.innerHTML = '<div class="cart-empty">Savatingiz bo\'sh</div>';
  ...
  keys.forEach((key) => {
    ...
    row
      ...
      .addEventListener("click", () => changeQty(key, -1));
    row
...
function renderOrders() {
  ...
  orders.forEach((order) => {
    const time = new Date(order.createdAt).toLocaleString("uz-UZ", {
      hour: "2-digit",
      minute: "2-digit",
      day: "2-digit",
      month: "2-digit",
    ...
    const itemsHtml = order.items
      .map(
        (i) =>
          `<div><span>${i.name} x${i.qty}</span><span>${formatSom(i.price * i.qty)}</span></div>`,
      )
      .join("");
    const card = document.createElement("div");
    ...
    card.querySelector(".status-select").addEventListener("change", (e) => {
      DB.updateOrderStatus(order.id, e.target.value);
      renderOrders();
    });
...
function renderMenuAdmin() {
  ...
  if (menu.length === 0) {
    grid.innerHTML =
      "<div class=\"empty-state\">Menyu bo'sh, pastdan taom qo'shing</div>";
    return;
  }
  menu.forEach((item) => {
    const priceHtml =
    ...
    card.querySelector('[data-act="delete"]').addEventListener("click", () => {
      if (confirm(`"${item.name}" ni menyudan butunlay o'chirmoqchimisiz?`)) {
...
function editItem(item) {
  ...
  document.getElementById("editSimplePriceField").style.display = sized
    ? "none"
    : "block";
  document.getElementById("editSizedFields").style.display = sized
    ? "block"
    : "none";
  if (sized) {
...
document.getElementById("newType").addEventListener("change", () => {
  ...
  document.getElementById("simplePriceField").style.display = sized
    ? "none"
    : "block";
  document.getElementById("size1Field").style.display = sized
    ? "block"
    : "none";
  document.getElementById("size2Field").style.display = sized
    ? "block"
    : "none";
  document.getElementById("size3Field").style.display = sized
    ? "block"
    : "none";
});
...
document.getElementById("addItemBtn").addEventListener("click", () => {
  ...
  if (!name) {
    showToast("Taom nomini kiriting");
    return;
  }
  ...
  try {
    if (type === "sized") {
      ...
      if ([p1, p2, p3].some((p) => isNaN(p) || p <= 0)) {
        showToast("1L, 1,5L, 2L narxlarini to'g'ri kiriting");
        return;
      }
      DB.addMenuItem({
        name,
      ...
      if (isNaN(price) || price <= 0) {
        showToast("To'g'ri narx kiriting");
        return;
      }
...
document.getElementById("saveCredsBtn").addEventListener("click", () => {
  ...
  if (!user || !pass) {
    showToast("Nik name va parolni to'liq kiriting");
    return;
  }
